...
const getRegistries = () => {
  const win = window;
  win._mfRegistries = win._mfRegistries || new Map();
  return win._mfRegistries;
};

const renderMicroFrontend = ({ history, name }) => {
  var _a;
  return (_a = getRegistries().get(name)) === null || _a === void 0
    ? void 0
    : _a.render(history);
};
...
const useMicroFrontend = ({ history, name }) =>
  useEffect(() => {
...
const isLoadedAsMicroFrontend = name => {
  var _a;
  return (
    name ===
    ((_a = glo[mfInfoKey]) === null || _a === void 0 ? void 0 : _a.name)
  );
};
...
const fetchScripts = (manifest, host, scriptId) =>
  new Promise(resolve => {
    let count = 0;
...
const lazyLoadMicroFrontend = ({ host, microFrontendName }) =>
  lazy(async () => {
    ...
    const Component = ({ history }) =>
      React.createElement(MicroFrontend, {
        history: history,
...
const MicroFrontendRouteComponent = _a => {
  var { host, microFrontendName } = _a,
    props = __rest(_a, ['host', 'microFrontendName']);
...
const MicroFrontendRoutesComponent = ({ fallback, routeProps }) =>
  React.createElement(
    Switch,
    null,
    React.createElement(
      ...
      routeProps.map(props =>
        React.createElement(
          MicroFrontendRoute,
          Object.assign({}, props, { key: props.microFrontendName }),
        ),
      ),
    ),
  );
